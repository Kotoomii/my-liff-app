...
def generate_dice_analysis():
    ...
    スケジューラーが23:30 JST（14:30 UTC）に実行したDiCE結果を読み取るだけ
    """
    try:
        data = request.get_json()
        user_id = data.get('user_id', 'default')
        ...
        if hourly_log.empty:
            return jsonify({
                ...
                'dice_analysis': {
                    'type': 'no_data',
                    'timeline': [],
                    'summary': 'まだDiCE提案が生成されていません。23:30以降に確認してください。'
                },
                'timestamp': datetime.now().isoformat()
            })
        ...
        if len(dice_suggestions) > 0:
            ...
            dice_result = {
                'type': 'no_suggestions',
                'timeline': [],
                'summary': 'DiCE提案はまだ生成されていません。23:30以降に確認してください。'
            }
...
def initialize_application():
    ...
    try:
        ...
        logger.warning("✅ 定期フィードバックスケジューラーを開始しました（毎日14:30 UTC = 23:30 JSTにDiCE実行 + フィードバック生成）")
