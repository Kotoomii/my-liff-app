...
class LLMFeedbackGenerator:
        ...
        try:
            ...
            data = {
                ...
                'messages': [
                    {
                        'role': 'system',
                        'content': 'あなたは優秀なストレス管理コンサルタントです。温かく、具体的で実践的なアドバイスを提供します。ユーザーの自律性を尊重し、命令形（「〜しましょう」「〜してください」）は絶対に使わず、提案型の表現（「〜してみるのはいかがでしょうか？」など）のみを使用してください。決定権は常にユーザーにあります。【重要】フィードバックは必ず250文字以内に収めてください。これは厳格な制約です。'
                    },
                    {
                        'role': 'user',
                        'content': prompt
            ...
            if response.status_code == 200:
                ...
                if len(generated_text) > 250:
                    logger.warning(f"⚠️ 生成されたフィードバックが250文字を超えています（{len(generated_text)}文字）。250文字に切り捨てます。")
                    generated_text = generated_text[:250]
                    # 末尾が中途半端な文にならないよう、句点で終わるように調整
                    last_period = max(generated_text.rfind('。'), generated_text.rfind('？'), generated_text.rfind('！'))
                    if last_period > 200:  # 200文字以上残る場合のみ
    ...
    def _build_daily_dice_feedback_prompt(self,
        ...
        """
        try:
            # 改善提案を予測NASA_F値の高い順にソート（ストレスが高い活動を優先）
            sorted_schedule = sorted(
                hourly_schedule,
                key=lambda x: x.get('original_frustration', 0),
                reverse=True
            )

            # 改善提案をテキスト化
            suggestions_text = []
            for suggestion in sorted_schedule[:3]:  # ストレスが高い上位3件
                ...
                original_f = suggestion.get('original_frustration', 0)

                suggestions_text.append(
                    f"- {time_range}: 「{original}」(予測ストレス{original_f:.1f}点)→「{suggested}」(改善: {improvement:.1f}点)"
                )
...
"""
            ...
            prompt = f"""
あなたはストレス管理の専門家です。ユーザーの自律性を尊重し、具体的な改善提案を提供してください。

## 今日のデータ
日付: {date}
平均フラストレーション: {avg_frustration:.1f}点
最もストレスが高かった活動: {highest_stress[0]} ({highest_stress_val:.1f}点)

## DiCE分析による改善提案（予測ストレスが高い順）
{chr(10).join(suggestions_text[:3]) if suggestions_text else '改善提案なし'}

## 重要な制約
1. **最もストレスが高い活動（予測NASA_F値が最も高い）とそのDiCE提案を優先してください**
   - 上記リストの**1番目（最もストレスが高い活動）**を必ず言及すること
   - 複数提案がある場合は、上位2つまで含めても良い

2. **DiCE提案された活動のみを使用してください**
   - 上記の「主な改善提案」に記載されている活動（suggested_activity）のみを提案すること
   - お風呂、音楽、散歩など、DiCE提案にない活動は絶対に提案しないこと

3. **具体的な活動名を必ず含めること**
   - DiCE提案の「suggested_activity」を具体的に言及すること
   - 例: 「睡眠」「運動」「食事」など、DiCE提案に記載されている活動名
   - **複数の提案がある場合は上位2つを含めてください**

4. **伝え方**
   - 「元の活動の後や間に、DiCE提案の活動を取り入れる」形式
   - 提案型の表現を使用（「〜してみるのはいかがでしょうか？」）
   - 命令形は禁止（「〜してください」「〜しましょう」）

## Few-shot例（テンプレート形式）

DiCE提案が1つの場合: 「[元の活動A]」→「[DiCE提案活動B]」(改善X点)
出力形式: 「今日は[元の活動A]でストレスを感じていましたね。[元の活動A]の後に[DiCE提案活動B]をしてみるのはいかがでしょうか？[DiCE提案活動Bの効果や特徴を1-2文で説明]。無理せず、自分のペースを大切にしてくださいね。」

DiCE提案が複数ある場合: 「[元の活動A]」→「[DiCE提案活動B]」(改善X点)、「[元の活動C]」→「[DiCE提案活動D]」(改善Y点)
出力形式: 「今日は[元の活動A]でストレスが高かったようですね。[元の活動A]の後に[DiCE提案活動B]をしてみるのはいかがでしょうか？また、[元の活動C]の際には[DiCE提案活動D]を取り入れてみるのも良いかもしれませんね。自分を大切にして、無理のないペースで過ごしてくださいね。」

**重要**: 上記の[元の活動A]、[DiCE提案活動B]などは、必ず「主な改善提案」に記載されている実際の活動名に置き換えてください。プレースホルダーのまま出力しないでください。

## 出力形式（250文字以内厳守）
- **番号付きリストは使用禁止**。自然な文章で書いてください
- 高ストレス活動への共感 → DiCE提案活動の具体的な提案（複数可） → 前向きな締めの言葉
- 流れるような自然な文章で、4-6文程度で構成してください
- 250文字に近い長さを目指してください
"""
    ...
    def _build_prediction_only_feedback_prompt(self,
        ...
        # 最も高ストレスと最も低ストレスの活動を特定
        highest_stress_activity = ""
        lowest_stress_activity = ""

        if not high_stress.empty:
            top_row = high_stress.iloc[0]
...
ユーザーの今日1日のフラストレーション推定値を振り返り、具体的なアドバイスを提供してください。
...
上記のデータをもとに、以下を含むフィードバックを提供してください：
1. 最もストレスが高かった活動を確認し、ストレスを感じていたことを共感的に伝える
2. 最もストレスが低かった活動を増やす・多くすることを提案する
3. 温かく前向きなトーン

重要な制約：
- 命令形（「〜しましょう」「〜してください」）は使わず、提案型の表現（「〜してみるのはいかがでしょうか？」など）を使用
- フィードバックは必ず250文字以内に収めてください（厳格な制約）
- 箇条書きは使わず、自然な文章で
"""
        return prompt
