...
## Few-shot例
DiCE提案: 「仕事」→「睡眠」(改善5.2点)の場合
出力: 「今日は仕事でストレスを感じていましたね。仕事の合間に15分程度の睡眠を取り入れてみるのはいかがでしょうか？少し休むことで、気持ちをリフレッシュできるかもしれません。」

DiCE提案: 「授業・学内の活動」→「インターネット動画」(改善4.3点)の場合
出力: 「授業・学内の活動でストレスが高かったようですね。授業の合間にインターネット動画を視聴してみるのはいかがでしょうか？新しい視点を取り入れてリフレッシュできるかもしれません。自分を大切にしてくださいね。」

## 出力形式（250文字以内厳守）
- **番号付きリストは使用禁止**。自然な文章で書いてください
- 高ストレス活動への共感 → DiCE提案活動の具体的な提案 → 前向きな締めの言葉
- 流れるような自然な文章で、3-4文程度で構成してください
"""
